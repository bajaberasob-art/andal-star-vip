...
# ================== استدعاء مكتبات الإشعارات (Web Push) ==================
...
# ================== مفاتيح الإشعارات (VAPID Keys) ==================
VAPID_PUBLIC_KEY = "BINKXL801WkmNxQZ2PVgJMw2VxLNAT0CNKAjPe-Nhv2JuCE1VJ7WvnAaPGk-skHyVMVsXGTo38THm6L7L7rPiX0"
VAPID_PRIVATE_KEY = "QFVlQ3cMNjWcs1IgtkjAJwj-HrGGPmrOWEFwMA7JSe8"
VAPID_CLAIMS = {
    "sub": "mailto:bajaberasob@gmail.com"
}
...
# جدول حفظ أجهزة الزبائن لاستقبال الإشعارات
...
# ================== دالة إرسال الإشعارات إلى هواتف الزبائن ==================
def send_web_push(user_id, title, body, url='/notifications'):
    devices = UserDevice.query.filter_by(user_id=user_id).all()
    for device in devices:
        try:
            sub_info = json.loads(device.subscription_json)
            webpush(
            ...
            # إذا مسح الزبون الإذن من المتصفح، يتم حذف جهازه من القاعدة لتخفيف الضغط
...
def add_points_advanced(user, amount_paid, reason="شراء باقة"):                                       
    ...
    # إرسال إشعار فور الحصول على نقاط
    send_web_push(user.id, "🎉 نقاط جديدة", f"مبروك! تمت إضافة {total_points} نقطة لرصيدك.")
...
with app.app_context():
    ...
    if not Package.query.first():
        ...
        db.session.commit()
    print("✅ تم تجهيز قاعدة البيانات وتحديثها!")
...
# مسار استقبال اشتراك جهاز الزبون لحفظه
...
def save_subscription():
    ...
    try:
        ...
        existing_device = UserDevice.query.filter_by(user_id=user_id, subscription_json=sub_json).first()
        if not existing_device:
            new_device = UserDevice(user_id=user_id, subscription_json=sub_json)
            db.session.add(new_device)
            db.session.commit()
            
        return jsonify({'status': 'success', 'message': 'تم ربط الجهاز'})
    except Exception as e:
        db.session.rollback()
...
def transfer_balance():
    ...
    try:                                                                                                  
        ...
        # إرسال إشعار للمستلم
        send_web_push(recipient.id, "💸 رصيد جديد!", f"وصلك تحويل بمبلغ {amount} ريال من {sender.full_name}")
...
def topup_customer(uid):
    ...
    try:                                                                                                  
        ...
        db.session.commit()
        send_web_push(user.id, "💰 شحن رصيد", f"تم إضافة {amount} ريال لرصيدك في عندل ستار.")
    except Exception as e:
        db.session.rollback()
...
def delete_user(uid):
    ...
    if user:
        try:                                                                                                  
            ...
            UserDevice.query.filter_by(user_id=user.id).delete() # تنظيف هواتف الزبون
...
def admin_give_points():                                                                              
    ...
    try:
        ...
        db.session.add(trans)
        db.session.commit()
        send_web_push(user.id, "🎁 هدية إدارية", f"تم منحك {points} نقطة إضافية!")
...
def add_offer():
    ...
    if request.method == 'POST':
        ...
        # إشعار لكل العملاء بالعرض الجديد (Web Push)
        users = User.query.filter_by(role='customer').all()
        for u in users:
            send_web_push(u.id, f"🔥 عرض جديد: {offer.title}", f"خصم {offer.discount}% لا تفوّت الفرصة!", url="/buy")
        ...
        try:
            if to_all:
                ...
                for user in users:
                    ...
                    db.session.add(notif)
                    send_web_push(user.id, title, message) # بث الهاتف!
            else:                                                                                                 
                user_id = int(request.form.get('user_id'))
                notif = Notification(user_id=user_id, title=title, message=message)
                db.session.add(notif)                                                                         
                send_web_push(user_id, title, message) # بث الهاتف!
            
            db.session.commit()
            flash('تم إرسال الإشعار بنجاح (داخل وخارج النظام)!', 'success')
        except:
            db.session.rollback()
            flash('حدث خطأ', 'error')                                                             
